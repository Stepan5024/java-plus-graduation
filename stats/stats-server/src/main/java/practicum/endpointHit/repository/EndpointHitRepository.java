package practicum.endpointHit.repository;
...
public interface EndpointHitRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query("select new practicum.viewStats.model.ViewStats(eh.app, eh.uri, count(distinct eh.ip)) " +
    ...
    @Query("select new practicum.viewStats.model.ViewStats(eh.app, eh.uri, count(eh.ip)) " +
            "from EndpointHit as eh " +
            "where eh.timestamp > :start and eh.timestamp < :end and eh.uri in (:uri) " +
            "group by eh.app, eh.uri " +
